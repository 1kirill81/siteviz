...
/* --- Color Switcher Logic (Expanding Menu) --- */
(function() {
    const container = document.querySelector('.color-switcher-container');
    if (!container) return;

    const toggle = container.querySelector('.color-switcher-toggle');
    ...
    container.querySelector('.color-options-wrapper').addEventListener('click', (e) => {
        e.stopPropagation();
    });

    // Load saved settings
    const savedNeon = localStorage.getItem('neon-color');
    const savedNeonRGB = localStorage.getItem('neon-color-rgb');
    const savedText = localStorage.getItem('text-color');
    const savedPromoNeon = localStorage.getItem('promo-neon-color');
    const savedPromoNeonRGB = localStorage.getItem('promo-neon-color-rgb');
    const savedPromoText = localStorage.getItem('promo-text-color');
    const savedPromoBG = localStorage.getItem('promo-bg-color');

    if (savedNeon && savedNeonRGB) {
        applyNeonColor(savedNeon, savedNeonRGB);
        updateActiveOption('neon', savedNeon);
    }
    if (savedText) {
        applyTextColor(savedText);
        updateActiveOption('text', savedText);
    }
    if (savedPromoNeon && savedPromoNeonRGB) {
        applyPromoNeonColor(savedPromoNeon, savedPromoNeonRGB);
        updateActiveOption('promo-neon', savedPromoNeon);
    }
    if (savedPromoText) {
        applyPromoTextColor(savedPromoText);
        updateActiveOption('promo-text', savedPromoText);
    }
    if (savedPromoBG) {
        applyPromoBGColor(savedPromoBG);
        updateActiveOption('promo-bg', savedPromoBG);
    }
    ...
    options.forEach(option => {
        option.addEventListener('click', () => {
            ...
            if (type === 'neon') {
                ...
                applyNeonColor(color, rgb);
                localStorage.setItem('neon-color', color);
                localStorage.setItem('neon-color-rgb', rgb);
            } else if (type === 'text') {
                applyTextColor(color);
                localStorage.setItem('text-color', color);
            } else if (type === 'promo-neon') {
                const rgb = option.getAttribute('data-rgb');
                applyPromoNeonColor(color, rgb);
                localStorage.setItem('promo-neon-color', color);
                localStorage.setItem('promo-neon-color-rgb', rgb);
            } else if (type === 'promo-text') {
                applyPromoTextColor(color);
                localStorage.setItem('promo-text-color', color);
            } else if (type === 'promo-bg') {
                applyPromoBGColor(color);
                localStorage.setItem('promo-bg-color', color);
            }

            updateActiveOption(type, color);
        });
    });

    function applyNeonColor(color, rgb) {
        root.style.setProperty('--neon-color', color);
        root.style.setProperty('--neon-color-rgb', rgb);
    }
    ...
    function applyPromoNeonColor(color, rgb) {
        root.style.setProperty('--promo-neon-color', color);
        root.style.setProperty('--promo-neon-rgb', rgb);
    }

    function applyPromoTextColor(color) {
        root.style.setProperty('--promo-text-color', color);
    }

    function applyPromoBGColor(color) {
        root.style.setProperty('--promo-bg-color', color);
    }

    function updateActiveOption(type, color) {
        const typeOptions = container.querySelectorAll(`.color-option[data-type="${type}"]`);
